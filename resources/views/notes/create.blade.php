	...
	<div class="row">
			<div class="col-12">
					<div class="card">
                        ...
                        <option value="{{ $id }}" @if (old('project_id') == $id)  selected @endif>{{ $project }}</option>
                    @endforeach
										</select>
										@error('project_id')
									...
									<div class="form-group">
										...
										<textarea id="note_text" name="note_text" class="form-control @error('note_text') is-invalid @enderror" required>{{ old('note_text') }}</textarea>
										@error('note_text')
											<span class="invalid-feedback" role="alert">
													<strong>{{ $message }}</strong>
											</span>
